...
from ..components import health_bar
...
class Level1(tools._State):
    ...
    def setup_bricks(self):
        ...
        self.fireshoot_group = pg.sprite.Group()
    ...
    def setup_enemies(self):
        ...
        enemy_group11 = pg.sprite.Group(self.bowser)
    ...
    def setup_checkpoints(self):
        ...
        check11 = checkpoint.Checkpoint(7760, "11")
        check12 = checkpoint.Checkpoint(8600, "12", 5, 6)
        check13 = checkpoint.Checkpoint(8775, "13")
        check14 = checkpoint.Checkpoint(2740, "secret_mushroom", 360, 40, 12)
    ...
    def update_all_sprites(self, keys):
        ...
        self.enemy_group.update(self.game_info, self.fireshoot_group, self.viewport)
                ...
                if checkpoint.name == str(i):
                    for index, enemy in enumerate(self.enemy_group_list[i - 1]):
                        if i != 11:
                            ...
                            enemy.rect.x = self.viewport.right - 100
    ...
    def adjust_sprite_positions(self):
        ...
        self.adjus_powershoot_position()
    ...
    def adjust_mario_position(self):
        ...
        if self.mario.rect.x > (self.viewport.x + c.SCREEN_WIDTH - 40):
            self.mario.rect.x = self.viewport.x + c.SCREEN_WIDTH - 40
    ...
    def test_if_mario_is_falling(self):
        ...
        if pg.sprite.spritecollideany(self.mario, test_collide_group) is None:
            if (
                ...
                self.mario.state == c.WALKING_TO_CASTLE
                or self.mario.state == c.END_OF_LEVEL_FALL
            ):
    ...
    def adjust_mario_for_y_enemy_collisions(self, enemy):
        ...
        if self.mario.y_vel > 0:
            ...
            if enemy.name == c.BOWSER:
                self.bowser.hp -= 4
                self.health_bar.get_damage(4)
                if self.bowser.hp <= 0:
    ...
    def adjus_powershoot_position(self):
        for fireshoot in self.fireshoot_group:
            if fireshoot.name == c.FIRESHOOT:
                self.adjust_fireshoot_position(fireshoot)
    ...
    def adjust_fireshoot_position(self, fireshoot):
        """Moves fireball along the x, y axes and checks for collisions"""
        if fireshoot.state == c.FLYING:
            fireshoot.rect.x += fireshoot.x_vel
            self.check_fireshoot_collisions(fireshoot)
        self.delete_if_off_screen(fireshoot)
    ...
    def check_fireshoot_collisions(self, fireshoot):
        """Fireball collisions along axis"""
        collide_group = pg.sprite.Group(
            self.ground_group,
            self.pipe_group,
            self.step_group,
        ...
        enemy_group = pg.sprite.Group(self.mario)

        collider = pg.sprite.spritecollideany(fireshoot, collide_group)
        enemy = pg.sprite.spritecollideany(fireshoot, enemy_group)

        if collider:
            fireshoot.kill()
            self.sprites_about_to_die_group.add(fireshoot)
            fireshoot.explode_transition()
        elif enemy:
            self.fireshoot_kill(fireshoot)
    ...
    def fireshoot_kill(self, fireshoot):
        """Kills enemy if hit with fireball"""
        setup.SFX["kick"].play()
        fireshoot.kill()
        self.sprites_about_to_die_group.add(fireshoot)
    ...
    def blit_everything(self, surface):
        ...
        self.fireshoot_group.draw(self.level)
    ...
    def check_if_fight_boss(self):
        if self.mario.rect.x > 7675 and self.bowser.hp > 0:
            self.viewport.x = 7760
